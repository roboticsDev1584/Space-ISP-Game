...
class ChooseMap : RenderableEntity {
    ...
    override func render(canvas:Canvas) {
        ...
        let words1 = Text(location:Point(x:canvasSize.center.x-790,y:200), text:"Each setting has unique features which affect the gameplay.")
        words1.font = "40pt Josefin Sans"
        canvas.render(words1)

        let words2 = Text(location:Point(x:canvasSize.center.x-150, y:300), text:"Saturn: Ships fight near the planet for 5 minutes (Press F).")
        ...
        let words5 = Text(location:Point(x:20, y:600), text:"Star: Players must defeat each other before the star turns into a black hole (Press Y).")
